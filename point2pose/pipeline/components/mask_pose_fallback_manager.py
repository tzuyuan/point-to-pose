    ...
    def apply(self, frame, fe_result, objects: list) -> None:
        ...
        for obj_id, obj in enumerate(objects):
            ...
            if stats["applied"]:
                ...
                # Keep rel_pose consistent with the corrected pose, but only where
                # the front end produced one at all (frame-to-frame mode). In
                # frame-to-map mode rel_pose is deliberately None, and the
                # optimizers read that as "no odometry constraint for this frame"
                # (`elif data.rel_pose is not None` -> BetweenFactorPose3). A
                # coarse mask-derived translation must not become such a factor.
                prev_rel_pose = fe_result.rel_poses.get(obj_id)
                prev_pose = np.asarray(getattr(obj, "pose", np.eye(4)), dtype=float)
                if prev_rel_pose is not None and prev_pose.shape == (4, 4):
