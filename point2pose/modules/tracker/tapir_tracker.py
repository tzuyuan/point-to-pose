...
class TapirTracker(Tracker):
    ...
    def __init__(self, config):
        ...
        # Number of PIPs refinement iterations per tracked frame (checkpoint was trained with
        # the default of 4). Lower values run refine_pips fewer times per frame (faster) at
        # the cost of track accuracy/stability -- not validated against this checkpoint, tune
        # empirically.
        num_pips_iter = int(config.get("num_pips_iter", 4))

        # TAPIR model
        self._model = tapir_model.TAPIR(
            pyramid_level=1, use_casual_conv=True, num_pips_iter=num_pips_iter
        )
