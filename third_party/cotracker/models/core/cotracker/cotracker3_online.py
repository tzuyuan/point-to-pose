...
from third_party.cotracker.models.core.model_utils import (
    sample_features5d,
    bilinear_sampler,
)
from third_party.cotracker.models.core.embeddings import (
    get_1d_sincos_pos_embed_from_grid,
)

from third_party.cotracker.models.core.cotracker.blocks import Mlp, BasicEncoder
from third_party.cotracker.models.core.cotracker.cotracker import EfficientUpdateFormer
...
class CoTrackerThreeOnline(CoTrackerThreeBase):
    ...
    def forward(
        ...
        # If we are online and already have caches, and N just increased, pad them now
        if is_online and (self.online_coords_predicted is not None):
            N_old = self.online_coords_predicted.size(2)
            if N > N_old:
        ...
        for i in range(self.corr_levels):
            ...
            if is_online:
                ...
                # <-- N might have grown this step; ensure caches are padded before +=
                if self.online_track_feat[i].size(2) < N:
                    self.online_track_feat[i] = self._pad_N(
                        self.online_track_feat[i], N, dimN=2, fill=0.0
                    )
                    self.online_track_support[i] = self._pad_N(
                        self.online_track_support[i], N, dimN=2, fill=0.0
                    )
    ...
    def _pad_N(self, t: torch.Tensor, newN: int, dimN: int, fill: float = 0.0):
        """
        Pad tensor t along its 'track' dimension to size newN (no-op if already >= newN).
        dimN is the axis index where N lives (e.g., 2 for (B,T,N,2), 1 for (B,N,D)).
        """
        if t is None:
            return None
        oldN = t.size(dimN)
        ...
        pad_shape[dimN] = newN - oldN
        pad = t.new_full(pad_shape, fill)
        return torch.cat([t, pad], dim=dimN)

    def _ensure_capacity_for_new_N(self, N_new: int):
        """
        Grow all online caches that carry N so we can add tracks dynamically.
        """
        # online predictions: (B,T,N,2), (B,T,N), (B,T,N)
        self.online_coords_predicted = self._pad_N(
            self.online_coords_predicted, N_new, dimN=2, fill=0.0
        )
        self.online_vis_predicted = self._pad_N(
            self.online_vis_predicted, N_new, dimN=2, fill=0.0
        )
        ...
        for i in range(self.corr_levels):
            ...
            self.online_track_support[i] = self._pad_N(
                self.online_track_support[i], N_new, dimN=2, fill=0.0
            )
